# Deliverable 4: Turn script into function
Write-Host "=== DELIVERABLE 4: SCRIPT TO FUNCTION ===" -ForegroundColor Green
Write-Host "Expected Outcome: Function takes 1 input variable and returns table of results" -ForegroundColor Green

# Get user input for number of days
$days = Read-Host "Enter number of days to search back"

$startDate = (Get-Date).AddDays(-$days)
$endDate = Get-Date
$loginLogoffEvents = @()
Write-Host "Searching from $($startDate.ToString('yyyy-MM-dd')) to $($endDate.ToString('yyyy-MM-dd'))..." -ForegroundColor Cyan
# Try to get EventID 7001/7002 from different logs (from user-specified days ago to now)
$events = @()
